...
Eleven rules, read before every draft. They are corrections collected from reviewed tickets, so on a
...
3. **Acceptance criteria are observable.** Each bullet names a state, an action and what is visible
   ...
   object. A value shaped like a credential, token, key or internal hostname is never copied from
   code into a ticket; cite the path.
