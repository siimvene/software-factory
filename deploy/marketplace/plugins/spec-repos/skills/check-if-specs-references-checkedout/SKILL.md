---
name: check-if-specs-references-checkedout
description: Use this skill when the user wants to check whether the git repositories referenced in a specs repo's `.tech-refs.md` files are checked out locally — e.g. "check if the repos referenced in the specs are checked out", "verify specs dependencies", "which repos does control-team-specs reference and are they cloned". Requires the name of a specs folder that is a sibling of the current project's root.
version: 1.0.0
---
...
See `../../references/spec-repos-concept.md` for the specs-repo convention this relies on (one specs repo, subdomain folders, paired `<slug>.md` + `<slug>.tech-refs.md` files).

**Argument**: The name of the specs folder (e.g. `control-team-specs`). This folder must be a sibling of the current project's root directory.
...
1. **Resolve paths**:
   ...
   Example:
   - project root = `/code/plg/ee-ai-hackathon`, argument = `control-team-specs`
   - `{git repos path}` = `/code/plg`
   - specs folder = `/code/plg/control-team-specs`
...
4. **Report**: Print:
   ...
   - A list of every referenced repo with ✓ (directory exists) or ✗ (directory missing)
