...
| Sensor 2, YAGNI ladder: seven rungs before code is written | ponytail, MIT, `DietrichGebert/ponytail`, installed as a marketplace plugin | same tool. Note it installs lifecycle hooks and cannot be revision-pinned through the marketplace format, so inspect the hooks and consider a fork | designed |
