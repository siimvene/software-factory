...
| Current-state specs generated from code, and kept current per commit | a plugin in the group's plugin marketplace: three skills, retro-generate from a codebase, update-specs-for-commits, and a checker that the specs' cited paths resolve in the checked-out tree; anonymised copies in this repository's [templates/skills](../templates/skills/), plus the three-check spec checker the reference implementation lost to a scratch directory | copy the templates; the checker is the part that must be mechanical and it ships as a script | measured |
...
| PM workspace: intake inbox, discovery notes, features, ticket drafts, decisions, stakeholder notes, and skills that read specs before writing a ticket | a plugin in the group's plugin marketplace (three skills: workspace setup, ingest, graduate) plus a folder template in the group standard; the five skills that produce the structured input (ticket, readiness-evaluator, task-improver, task-splitter, graduate) ship anonymised in [templates/skills](../templates/skills/) | copy the five; the folder is a folder; the value is the ordering rule that a ticket is drafted against the specs, not against the code | measured (with a finding: the template's skills referenced files the copy did not have) |
| Program design note per sized ticket: modules and lanes, types, signatures, shape decisions, slice order, tests red on the base | [templates/program-design.md](../templates/program-design.md); the code map a scout produced and the API contract the principal wrote in cycle 4 are its measured precursors | copy the template; the spillover check and the fail-on-base check are what make it more than a document | proposed (template) / measured (the cycle-4 contract) |
...
| Fail-on-base check: new or changed tests run at the merge base with the branch's tests applied, must fail there and pass on the branch, exemptions declared, failure kind recorded | nothing yet; the gate report's `red-before` row is a hand step | a script of 50 to 150 lines per stack: select test ids from the diff, a temporary worktree at the merge base, run twice, write the receipt | proposed |
